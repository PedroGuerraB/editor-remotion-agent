...
function slideUp(frame, duration = 20) {
  return interpolate(frame, [0, duration], [60, 0], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
    easing: Easing.bezier(0.16, 1, 0.3, 1),
  });
...
function AttentionSection({ frame, fps }) {
  ...
  return (
    <AbsoluteFill style={{
      background: 'linear-gradient(180deg, #0a0000 0%, #1a0000 50%, #0a0000 100%)',
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 36,
    }}>
      <div style={{
        background: '#FF2D2D', width: '100%', padding: '32px 0', textAlign: 'center',
        fontSize: 68, fontWeight: 900, color: '#fff', letterSpacing: 4,
        transform: `scale(${bannerScale})`, boxShadow: '0 8px 48px rgba(255,45,45,0.6)',
      }}>⚠️ ATENÇÃO ⚠️</div>
      <div style={{
        opacity: subOpacity, transform: `translateY(${subY}px)`,
        fontSize: 52, color: '#fff', fontWeight: 700, textAlign: 'center', padding: '0 60px', lineHeight: 1.35,
      }}>
        Material disponível em<br />
        <span style={{ color: '#FFE600', fontSize: 60 }}>condição PROMOCIONAL</span>
      </div>
      <div style={{
        border: '4px solid #FF2D2D', borderRadius: 32, padding: '44px 90px',
        display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8,
        transform: `scale(${priceScale * pricePulse})`,
        background: 'rgba(0,0,0,0.55)', boxShadow: '0 0 60px rgba(255,45,45,0.25)',
      }}>
...
function UrgencySection({ frame, fps }) {
  ...
  return (
    <AbsoluteFill style={{
      background: 'linear-gradient(180deg, #0a0200 0%, #1a0800 50%, #0a0200 100%)',
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 44, padding: '0 50px',
    }}>
      <div style={{ fontSize: 54, color: '#fff', fontWeight: 700, textAlign: 'center', lineHeight: 1.4, transform: `scale(${titleScale})` }}>
        Quando a promoção encerrar...
      </div>
      ...
      <div style={{ opacity: ctaOpacity, transform: `translateY(${ctaY}px)`, fontSize: 48, color: '#FFE600', fontWeight: 700, textAlign: 'center', textShadow: '0 0 24px rgba(255,230,0,0.4)' }}>
        🔥 Garanta agora pelo menor preço!
      </div>
    </AbsoluteFill>
  );
}
...
function CTASection({ frame, fps }) {
  ...
  const badgeOpacity = fadeIn(frame - 20);
  const badgeY       = slideUp(frame - 20);
  const footerOpacity = fadeIn(frame - 45);

  return (
    <AbsoluteFill style={{
      background: 'linear-gradient(180deg, #001400 0%, #002800 50%, #001400 100%)',
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 52, padding: '0 50px',
    }}>
      <div style={{ fontSize: 54, color: '#fff', fontWeight: 700, textAlign: 'center', lineHeight: 1.4 }}>
        Envio <span style={{ color: '#00E676' }}>TODO o material</span><br />antes mesmo do pagamento
      </div>
      <div style={{
        background: 'linear-gradient(135deg, #00C853, #00E676)', borderRadius: 64, padding: '38px 76px',
        fontSize: 58, fontWeight: 900, color: '#fff',
        boxShadow: '0 8px 48px rgba(0,200,83,0.55), 0 0 0 6px rgba(0,200,83,0.25)',
        transform: `scale(${buttonScale * buttonPulse})`, textAlign: 'center', letterSpacing: -1,
      }}>👆 CLIQUE EM SAIBA MAIS</div>
      <div style={{ opacity: badgeOpacity, transform: `translateY(${badgeY}px)`, background: 'rgba(0,0,0,0.65)', borderRadius: 44, padding: '24px 48px', fontSize: 44, color: '#fff', fontWeight: 600, textAlign: 'center', border: '2px solid rgba(255,255,255,0.15)' }}>
        📲 Receba tudo no WhatsApp<br />
        <span style={{ fontSize: 34, color: '#aaa', fontWeight: 400 }}>antes de qualquer pagamento</span>
      </div>
      <div style={{ opacity: footerOpacity, fontSize: 38, color: '#666', textAlign: 'center', fontWeight: 500 }}>Sem compromisso ✅</div>
    </AbsoluteFill>
  );
}
